/*
[IMPORTANT]
You are free to create any number of helper function you want.
We know the problem could be seached online, and we are aware of those solutions.
So please sight sources if you took help from any online resource.
*/



//IDs for all the table elements. You get the cell element just by using document.getElementById("A1")
var table_ids = ["A1", "A2", "A3", "B1", "B2", "B3", "C1", "C2", "C3"]

/*
An integer array of length 9.
Usaged: This is to store the state to the tictactoe board.
When a move is made
(Example player 1 (who is X) move at Cell 'A1' --- The board_state[0] will be made 1 )
Similarly, A move by player 2(who is O) at Cell 'A3' --- The board_state[2] will be made 0 )
We store the move of player 1 as '1' and player 2 as '0'. So after the above two moves the state should look like
[1, -1, 0, -1, -1, -1, -1, -1, -1]
*/
var board_state = [-1,-1,-1,-1,-1,-1,-1,-1,-1]


// A flag to keep track of the status of the game, false means the game is not started. The default value is set to false
var started = false

/*
A variable to keep track of each players turn. Since the game always starts with player 1 - The default value is set to '1'
1 means player_1
0 means player_0
*/
var turn = 1

/*
 @Return boolean
 @Param _str - A string variable - Note the type is not checked in the implementation
 The methods @Returns true is the _str is null or it has a length of 0, otherwise, the methods returns false
*/
function isEmpty(_str) {
	return (!_str || 0 === _str.length)
}

/*
@Return int This return the turn variable. Please note that
turn = 1 is for player_1 and
turn = 0 is for player_2
@Param - No param
*/
function whose_move(){
	return this.turn
}

/*
@Return void
@Param
This methods toggles the 'turn' variable.
if the turn is set to 1 it will make it 0
if the turn is set to 0 it will make it 1
*/
function toggle_move() {
	this.turn = !this.turn
}

/*
@Return boolean
@Param
The method returns the value of the 'started' flag.
true means the game has started
false means the game has not started
When the game has not started the flag is set to false. As soon as the game starts the flag must be set to true.
Once the game has finished or user has clicked on reset_play the flag must be set to false.
*/
function game_started(){
	return this.started
}


/*
TODO - Rule 1
This is the first method you'll implement. This method is called when the Begin Play button is clicked.
The method should do all the validations as stated in rule 1.
1. Verify if the player names are empty or not. Raise an alert if they are empty.
2. If the field are empty don't start the game. This just means the function will return and do nothing. The 'started' flag will not be modified.
3. If all verification is successful, disable the name fields and update the player moves as shown in the image.
4. If all verification is successful, update the turn information on the page. (See the source code and image). And set the started flag to true.(this will help you track at any instant if the game is in start state or not.)
5. Once game has started, Handle multiple clicks on begin play.
*/
function begin_play(){
	if(isEmpty(document.getElementById("player1_id").value) || isEmpty(document.getElementById("player2_id").value))
	{
		alert("Enter something in the player field please.")
		started = false
		return
	}
	//diables button begin play.
	else if (game_started() == true) {
		alert("Bad Monkey dont do that!")
	}
	else {
		//disable the name fields and update the player moves as shown in the image
		document.getElementById("player1_id").disabled = true
		document.getElementById("player2_id").disabled = true
		var player_1 = document.getElementById("player1_id")
		player_1.value = player_1.value + " (X) "
		var player_2 = document.getElementById("player2_id")
		player_2.value = player_2.value + " (O) "

		//changes text
		var updateTurn = document.getElementById("turn_info")
		updateTurn.innerHTML = "Player X Turn"

		started = true;
	}
}

/*
TODO - Rule 2
This is the second method you'll implement. This method is called when the Reset Play button is clicked.
The method should do all the things as stated in rule 2.
1. The reset play button should reset the whole game.(At any time when reset is clicked - All the three text boxes should be cleared and Turn should be set to the default message.)
2. The text boxes for entering name should be enablled back.
3. The Tic Tac Toe Grid should be set to its default entries.
4. Clicking reset play again and again shall have the same effect.(or no effect when clicked multiple times)
Remember to set the strated flag as false

*/
function reset_play(){

	var player1Id = document.getElementById("player1_id")
	player1Id.value = ""
	var player2Id = document.getElementById("player2_id")
	player2Id.value = ""
	var updateTurn = document.getElementById("turn_info")
	updateTurn.innerHTML = "Game has not begun."
	var move = document.getElementById("move_text_id")
	move.value = ""

	document.getElementById("player1_id").disabled = false
	document.getElementById("player2_id").disabled = false

	var a1 = document.getElementById("A1")
	var a2 = document.getElementById("A2")
	var a3 = document.getElementById("A3")

	a1.innerHTML = "A1"
	a2.innerHTML = "A2"
	a3.innerHTML = "A3"

	var b1 = document.getElementById("B1")
	var b2 = document.getElementById("B2")
	var b3 = document.getElementById("B3")

	b1.innerHTML = "B1"
	b2.innerHTML = "B2"
	b3.innerHTML = "B3"

	var c1 = document.getElementById("C1")
	var c2 = document.getElementById("C2")
	var c3 = document.getElementById("C3")

	c1.innerHTML = "C1"
	c2.innerHTML = "C2"
	c3.innerHTML = "C3"

	board_state = [-1,-1,-1,-1,-1,-1,-1,-1,-1]
	started = false
	turn = 1

	begin_play()

}

function WinnerWinnerChickenDinner()
{
	var a1 = document.getElementById("A1")
	var a2 = document.getElementById("A2")
	var a3 = document.getElementById("A3")

	var b1 = document.getElementById("B1")
	var b2 = document.getElementById("B2")
	var b3 = document.getElementById("B3")

	var c1 = document.getElementById("C1")
	var c2 = document.getElementById("C2")
	var c3 = document.getElementById("C3")

	var move = document.getElementById("move_text_id")

	if( (a1.innerText == "X" && a2.innerText == "X" && a3.innerText == "X") || (a1.innerText == "O" && a2.innerText == "O" && a3.innerText == "O") )
	{
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}
	else if ( (b1.innerText == "X" && b2.innerText == "X" && b3.innerText == "X") || (b1.innerText == "O" && b2.innerText == "O" && b3.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}
	else if ( (c1.innerText == "X" && c2.innerText == "X" && c3.innerText == "X") || (c1.innerText == "O" && c2.innerText == "O" && c3.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}

	else if ( (a1.innerText == "X" && b1.innerText == "X" && c1.innerText == "X") || (a1.innerText == "O" && b1.innerText == "O" && c1.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}
	else if ( (a2.innerText == "X" && b2.innerText == "X" && c2.innerText == "X") || (a2.innerText == "O" && b2.innerText == "O" && c2.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}
	else if ( (a3.innerText == "X" && b3.innerText == "X" && c3.innerText == "X") || (a3.innerText == "O" && b3.innerText == "O" && c3.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}

	else if ( (a1.innerText == "X" && b2.innerText == "X" && c3.innerText == "X") || (a1.innerText == "O" && b2.innerText == "O" && c3.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}
	else if ( (a3.innerText == "X" && b2.innerText == "X" && c1.innerText == "X") || (a3.innerText == "O" && b2.innerText == "O" && c1.innerText == "O") ) {
		whose_move() == 1 ? alert("Player X Wins!") : alert("Player X Wins!")
		reset_play()
	}
	else if (
		 a1.innerText != "A1" && a2.innerText != "A2"&& a3.innerText != "A3" &&
		 b1.innerText != "B1" && b2.innerText != "B2"&& move.value != "B3" &&
		 c1.innerText != "C1" && c2.innerText != "C2"&& move.value != "C3")
		{
			alert("Game Tied!")
			reset_play()
		}
}

/*
TODO - Rule 3
This is the last method you'll implement. This method is called everytime a move has been player( Play button was clicked).
The method should do all the things as stated in rule 2.
1. The moves should be validated can only be these ["A1", "A2", "A3", "B1", "B2", "B3", "C1", "C2", "C3"]
2. Invalid moves should be reported by an alert message.(You are encorraged to use Modal which you learned in HW1 - Usage is not mandatory.)
3. If the move is a valid move, the grid should be updated with the correct move (Player 1 is always - 'X', and Player 2 is always 'O' (This is not zero!)) - The turn information should also be updated
	Hint: Use the turn variable to figure out who is currently playing. Use to toggle method to change moves.
4. A move should always be a valid move. (Example: If say a move was made in already filled cell, it should be invalidated with an alert.)
5. If the game has not started, clicking on <b>Play</b> should give an alert "The game has not started."<br/>

//TODO
6. After any move, the state of the table should be validated.(see the document attached in the homework)
   If the there is winner - Show it in an alert message - (Ex - Winner is X or O) - Displaying name is not important. <br/>
7. The game should reset itself once a winner is determined.<br/>
8. After all the moves have exhausted, you're not required to display any message. (It should be obvious to Reset play.)<br/>

*/
function play() {
	var a1 = document.getElementById("A1")
	var a2 = document.getElementById("A2")
	var a3 = document.getElementById("A3")

	var b1 = document.getElementById("B1")
	var b2 = document.getElementById("B2")
	var b3 = document.getElementById("B3")

	var c1 = document.getElementById("C1")
	var c2 = document.getElementById("C2")
	var c3 = document.getElementById("C3")

	var move = document.getElementById("move_text_id")

	//changes text
	var updateTurn = document.getElementById("turn_info")

	if(game_started() == false){
		alert("The game has not started.")
	}
	if(move.value == "A1" || move.value == "A2"|| move.value == "A3" ||
		 move.value == "B1" || move.value == "B2"|| move.value == "B3" ||
		 move.value == "C1" || move.value == "C2"|| move.value == "C3")
	{
		//console.log(move.value)
		if(move.value == "A1")
		{
			if(board_state[0] != 0)
			{
				a1.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				a1.inntHTML = whose_move() ? board_state[0] = 0 : board_state[0] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"

			}
			else {
				alert("A1 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}
		else if  (move.value == "A2"){
			if(board_state[1] != 0)
			{
				a2.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				a2.inntHTML = whose_move() ? board_state[1] = 0 : board_state[1] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("A2 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}
		else if  (move.value == "A3"){
			if(board_state[2] != 0)
			{
				a3.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				a3.inntHTML = whose_move() ? board_state[2] = 0 : board_state[2] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("A3 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}

		else if(move.value == "B1")
		{
			if(board_state[3] != 0)
			{
				b1.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				b1.inntHTML = whose_move() ? board_state[3] = 0 : board_state[3] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("B1 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}
		else if(move.value == "B2")
		{
			if(board_state[4] != 0)
			{
				b2.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				b2.inntHTML = whose_move() ? board_state[4] = 0 : board_state[4] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("B2 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}
		else if(move.value == "B3")
		{
			if(board_state[5] != 0)
			{
				b3.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				b3.inntHTML = whose_move() ? board_state[5] = 0 : board_state[5] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("B3 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}

		else if(move.value == "C1")
		{
			if(board_state[6] != 0)
			{
				c1.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				c1.inntHTML = whose_move() ? board_state[6] = 0 : board_state[6] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("C1 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}
		else if(move.value == "C2")
		{
			if(board_state[7] != 0)
			{
				c2.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				c2.inntHTML = whose_move() ? board_state[7] = 0 : board_state[7] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("C2 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
		}
		else if(move.value == "C3")
		{
			if(board_state[8] != 0)
			{
				c3.innerHTML = whose_move() ? "X" : "O"
				move.value = ""
				c3.inntHTML = whose_move() ? board_state[8] = 0 : board_state[8] = 1
				toggle_move()
				updateTurn.innerHTML = whose_move() ? "Player X Turn" : "Player O Turn"
			}
			else {
				alert("C3 was already selected, please choose another valid space!")
				move.value = "" //clears text field
			}
			//updateTurn.innerHTML = toggle_move() ? "Player X Turn" : "Player O Turn"
		}
	}
	else {
		alert("Invalid Move, please try again!")
		move.value = "" //clears text field
	}
  WinnerWinnerChickenDinner()

}

/*
Do not change this method.
*/
function moveEnter(event) {
	if(event.keyCode == 13) {
		event.preventDefault()
		play()
	}

}
